...
def filter_denovo(metrics, cutoffs):
    """
    ...
    Yields
    ------
    name, quad, samples : str, str, str
    """

    # Get metrics of each family called in each variant
    grouped = itertools.groupby(metrics, key=lambda m: (m.name, m.quad))

    for (name, quad), samples in grouped:
        proband = sibling = None
        ...
        tests = []
        if proband:
            tests.append(DenovoFilter(proband, father, mother, cutoffs))
        if sibling:
            tests.append(DenovoFilter(sibling, father, mother, cutoffs))
        ...
        yield (name, quad, ','.join(called_samples), support)


def metric_parser(metricfile):
    """Convert table of variant metrics to iterator over Metrics tuples"""
    header = next(metricfile).strip().split()
    Metrics = namedtuple('Metrics', ['quad', 'member'] + header)
    ...
    for line in metricfile:
        ...
        quad, member = data[1].split('.')
        yield Metrics(quad, member, *_cast(data))
...
def main():
    ...
    metrics = metric_parser(args.metrics)

    cutoffs = pd.read_table(args.cutoffs)
    cutoffs = cutoffs.drop('direction', axis=1)
    cutoffs = cutoffs.pivot_table(index='source svtype'.split(),
                                  values='cutoff', columns='metric')

    fmt = '{0}\t{1}\t{2}\t{3}\n'
    for name, quad, samples, support in filter_denovo(metrics, cutoffs):
        args.fout.write(fmt.format(name, quad, samples, support))
